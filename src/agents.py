...
class QueryAgent:
    ...
    def answer_query(self, query: str) -> dict:
        ...
        if category == "SIMPLE":
            ...
            graph_relations = self.graph_store.traverse_subgraph(high_deg_nodes, max_depth=1)
        else:  # HYBRID
            vector_chunks = self.vector_store.search(query, top_k=4)
            seeds = self.graph_store.find_seeds_in_query(query)
            if not seeds:
        ...
        # 4. Generate Answer and Self-Reflected Confidence
...
"""
        ...
        try:
            response_json = llm.generate_json(qa_prompt, system_instruction=qa_system_instruction)
            ...
            # Fallback if JSON parsing fails twice
            ...
            answer = llm.generate_text(fallback_prompt)
            confidence = 65
